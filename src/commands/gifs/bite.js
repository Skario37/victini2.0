...
exports.run = async (client, message, args, settings) => {
  ...
  // Gfycat
  const dataGfycat = await gfycat.search({"search_text": "bite", "count": 10}).then(d => {
  ...
  // Giphy
  const dataGiphy = await giphy.search({"q": "bite", "limit": 20, "rating": "pg", "fmt": "json"}).then(d => {
  ...
  // Tenor
  const dataTenor = await Tenor.Search.Query("bite", "15").then(d => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_BITE_SOMETHING", settings.language)
          .replace("{{emoji}}", Emoji.BITE.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
          .replace("{{variable}}", args)
      );
    } else {
      embed.setTitle(
        i18n("GIF_BITE_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.BITE.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
...
exports.help = (language) => { 
  return {
    name: i18n("HELP_BITE_NAME", language),
    category: i18n("HELP_BITE_CATEGORY", language),
    description: i18n("HELP_BITE_DESCRIPTION", language),
    usage: i18n("HELP_BITE_USAGE", language)
  }
}