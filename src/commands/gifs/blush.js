...
exports.run = async (client, message, args, settings) => {
  ...
  // Gfycat
  const dataGfycat = await gfycat.search({"search_text": "blush", "count": 10}).then(d => {
  ...
  // Giphy
  const dataGiphy = await giphy.search({"q": "blush", "limit": 20, "rating": "pg", "fmt": "json"}).then(d => {
  ...
  // Tenor
  const dataTenor = await Tenor.Search.Query("blush", "15").then(d => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_BLUSH_SOMETHING", settings.language)
          .replace("{{emoji}}", Emoji.BLUSH.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
          .replace("{{variable}}", args)
      );
    } else {
      embed.setTitle(
        i18n("GIF_BLUSH_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.BLUSH.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
...
exports.help = (language) => { 
  return {
    name: i18n("HELP_BLUSH_NAME", language),
    category: i18n("HELP_BLUSH_CATEGORY", language),
    description: i18n("HELP_BLUSH_DESCRIPTION", language),
    usage: i18n("HELP_BLUSH_USAGE", language)
  }
}