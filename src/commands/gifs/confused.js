...
exports.run = async (client, message, args, settings) => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_CONFUSED_SOMETHING", settings.language)
          .replace("{{emoji}}", Emoji.CONFUSED.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
          .replace("{{variable}}", args)
      );
    } else {
      embed.setTitle(
        i18n("GIF_CONFUSED_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.CONFUSED.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
...
exports.help = (language) => { 
  return {
    name: i18n("HELP_CONFUSED_NAME", language),
    category: i18n("HELP_CONFUSED_CATEGORY", language),
    description: i18n("HELP_CONFUSED_DESCRIPTION", language),
    usage: i18n("HELP_CONFUSED_USAGE", language)
  }
}