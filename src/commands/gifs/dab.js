...
exports.run = async (client, message, args, settings) => {
  ...
  // Gfycat
  const dataGfycat = await gfycat.search({"search_text": "dab", "count": 10}).then(d => {
  ...
  // Giphy
  const dataGiphy = await giphy.search({"q": "dab", "limit": 20, "rating": "pg", "fmt": "json"}).then(d => {
  ...
  // Tenor
  const dataTenor = await Tenor.Search.Query("dab", "15").then(d => {
...
exports.help = (language) => { 
  return {
    name: i18n("HELP_DAB_NAME", language),
    category: i18n("HELP_DAB_CATEGORY", language),
    description: i18n("HELP_DAB_DESCRIPTION", language),
    usage: i18n("HELP_DAB_USAGE", language)
  }
}