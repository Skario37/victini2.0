...
exports.run = async (client, message, args, settings) => {
  ...
  // Gfycat
  const dataGfycat = await gfycat.search({"search_text": "dance", "count": 10}).then(d => {
  ...
  // Giphy
  const dataGiphy = await giphy.search({"q": "dance", "limit": 20, "rating": "pg", "fmt": "json"}).then(d => {
  ...
  // Tenor
  const dataTenor = await Tenor.Search.Query("dance", "15").then(d => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_DANCE_SOMETHING", settings.language)
          .replace("{{emoji}}", Emoji.DANCE.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
          .replace("{{variable}}", args)
      );
    } else {
      embed.setTitle(
        i18n("GIF_DANCE_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.DANCE.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
...
exports.conf = {
  ...
  enabled: true,
  guildOnly: false,
  aliases: ["danse, danser"]
}

exports.help = (language) => { 
  return {
    name: i18n("HELP_DANCE_NAME", language),
    category: i18n("HELP_DANCE_CATEGORY", language),
    description: i18n("HELP_DANCE_DESCRIPTION", language),
    usage: i18n("HELP_DANCE_USAGE", language)
  }
}