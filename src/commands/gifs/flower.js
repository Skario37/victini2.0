...
exports.run = async (client, message, args, settings) => {
  ...
  // Gfycat
  const dataGfycat = await gfycat.search({"search_text": "flower", "count": 10}).then(d => {
  ...
  // Giphy
  const dataGiphy = await giphy.search({"q": "flower", "limit": 20, "rating": "pg", "fmt": "json"}).then(d => {
  ...
  // Tenor
  const dataTenor = await Tenor.Search.Query("flower", "15").then(d => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_FLOWER_SOMETHING", settings.language)
          .replace("{{emoji}}", Emoji.FLOWER.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
          .replace("{{variable}}", args)
      );
    } else {
      embed.setTitle(
        i18n("GIF_FLOWER_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.FLOWER.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
...
exports.conf = {
  ...
  enabled: true,
  guildOnly: false,
  aliases: ["flowers", "fleur", "fleurs"]
}

exports.help = (language) => { 
  return {
    name: i18n("HELP_FLOWER_NAME", language),
    category: i18n("HELP_FLOWER_CATEGORY", language),
    description: i18n("HELP_FLOWER_DESCRIPTION", language),
    usage: i18n("HELP_FLOWER_USAGE", language)
  }
}