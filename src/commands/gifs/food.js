...
exports.run = async (client, message, args, settings) => {
  ...
  // Gfycat
  const dataGfycat = await gfycat.search({"search_text": "food", "count": 10}).then(d => {
  ...
  // Giphy
  const dataGiphy = await giphy.search({"q": "food", "limit": 20, "rating": "pg", "fmt": "json"}).then(d => {
  ...
  // Tenor
  const dataTenor = await Tenor.Search.Query("food", "15").then(d => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_FOOD_SOMETHING", settings.language)
          .replace("{{emoji}}", Emoji.FOOD.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
          .replace("{{variable}}", args)
      );
    } else {
      embed.setTitle(
        i18n("GIF_FOOD_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.FOOD.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
...
exports.conf = {
  ...
  enabled: true,
  guildOnly: false,
  aliases: ["eat", "nourriture", "manger"]
}

exports.help = (language) => { 
  return {
    name: i18n("HELP_FOOD_NAME", language),
    category: i18n("HELP_FOOD_CATEGORY", language),
    description: i18n("HELP_FOOD_DESCRIPTION", language),
    usage: i18n("HELP_FOOD_USAGE", language)
  }
}