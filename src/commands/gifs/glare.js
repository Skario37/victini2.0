...
exports.run = async (client, message, args, settings) => {
  ...
  // Gfycat
  const dataGfycat = await gfycat.search({"search_text": "glare", "count": 10}).then(d => {
  ...
  // Giphy
  const dataGiphy = await giphy.search({"q": "glare", "limit": 20, "rating": "pg", "fmt": "json"}).then(d => {
  ...
  // Tenor
  const dataTenor = await Tenor.Search.Query("glare", "15").then(d => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_GLARE_SOMETHING", settings.language)
          .replace("{{emoji}}", Emoji.GLARE.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
          .replace("{{variable}}", args)
      );
    } else {
      embed.setTitle(
        i18n("GIF_GLARE_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.GLARE.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
...
exports.conf = {
  ...
  enabled: true,
  guildOnly: false,
  aliases: ["stare", "fixer", "regarder"]
}

exports.help = (language) => { 
  return {
    name: i18n("HELP_GLARE_NAME", language),
    category: i18n("HELP_GLARE_CATEGORY", language),
    description: i18n("HELP_GLARE_DESCRIPTION", language),
    usage: i18n("HELP_GLARE_USAGE", language)
  }
}