...
exports.run = async (client, message, args, settings) => {
  ...
  // Gfycat
  const dataGfycat = await gfycat.search({"search_text": "kiss", "count": 10}).then(d => {
  ...
  // Giphy
  const dataGiphy = await giphy.search({"q": "kiss", "limit": 20, "rating": "pg", "fmt": "json"}).then(d => {
  ...
  // Tenor
  const dataTenor = await Tenor.Search.Query("kiss", "15").then(d => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_KISS_SOMETHING", settings.language)
          .replace("{{emoji}}", Emoji.KISS.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
          .replace("{{variable}}", args)
      );
    } else {
      embed.setTitle(
        i18n("GIF_KISS_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.KISS.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
...
exports.conf = {
  ...
  enabled: true,
  guildOnly: false,
  aliases: ["kisses", "bisou", "bisous"]
}

exports.help = (language) => { 
  return {
    name: i18n("HELP_KISS_NAME", language),
    category: i18n("HELP_KISS_CATEGORY", language),
    description: i18n("HELP_KISS_DESCRIPTION", language),
    usage: i18n("HELP_KISS_USAGE", language)
  }
}