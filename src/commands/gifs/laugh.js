...
exports.run = async (client, message, args, settings) => {
  ...
  // Gfycat
  const dataGfycat = await gfycat.search({"search_text": "laugh", "count": 10}).then(d => {
  ...
  // Giphy
  const dataGiphy = await giphy.search({"q": "laugh", "limit": 20, "rating": "pg", "fmt": "json"}).then(d => {
  ...
  // Tenor
  const dataTenor = await Tenor.Search.Query("laugh", "15").then(d => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_LAUGH_SOMETHING", settings.language)
          .replace("{{emoji}}", Emoji.LAUGH.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
          .replace("{{variable}}", args)
      );
    } else {
      embed.setTitle(
        i18n("GIF_LAUGH_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.LAUGH.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
...
exports.help = (language) => { 
  return {
    name: i18n("HELP_LAUGH_NAME", language),
    category: i18n("HELP_LAUGH_CATEGORY", language),
    description: i18n("HELP_LAUGH_DESCRIPTION", language),
    usage: i18n("HELP_LAUGH_USAGE", language)
  }
}