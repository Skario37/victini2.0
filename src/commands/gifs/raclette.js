...
exports.run = async (client, message, args, settings) => {
  ...
  // Gfycat
  const dataGfycat = await gfycat.search({"search_text": "raclette", "count": 10}).then(d => {
  ...
  // Giphy
  const dataGiphy = await giphy.search({"q": "raclette", "limit": 20, "rating": "pg", "fmt": "json"}).then(d => {
  ...
  // Tenor
  const dataTenor = await Tenor.Search.Query("raclette", "15").then(d => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_RACLETTE_SOMETHING", settings.language)
          .replace("{{emoji}}", Emoji.RACLETTE.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
          .replace("{{variable}}", args)
      );
    } else {
      embed.setTitle(
        i18n("GIF_RACLETTE_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.RACLETTE.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
    ...
    return message.reply({ content: embed, allowedMentions: { repliedUser: false}}).catch(e => {});
  } else {
...
exports.help = (language) => { 
  return {
    name: i18n("HELP_RACLETTE_NAME", language),
    category: i18n("HELP_RACLETTE_CATEGORY", language),
    description: i18n("HELP_RACLETTE_DESCRIPTION", language),
    usage: i18n("HELP_RACLETTE_USAGE", language)
  }
}