...
exports.run = async (client, message, args, settings) => {
  ...
  // Gfycat
  const dataGfycat = await gfycat.search({"search_text": "sad", "count": 10}).then(d => {
  ...
  // Giphy
  const dataGiphy = await giphy.search({"q": "sad", "limit": 20, "rating": "pg", "fmt": "json"}).then(d => {
  ...
  // Tenor
  const dataTenor = await Tenor.Search.Query("sad", "15").then(d => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_SAD_SOMETHING", settings.language)
          .replace("{{emoji}}", Emoji.SAD.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
          .replace("{{variable}}", args)
      );
    } else {
      embed.setTitle(
        i18n("GIF_SAD_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.SAD.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
...
exports.conf = {
  ...
  enabled: true,
  guildOnly: false,
  aliases: ["cry", "triste", "pleurer"]
}

exports.help = (language) => { 
  return {
    name: i18n("HELP_SAD_NAME", language),
    category: i18n("HELP_SAD_CATEGORY", language),
    description: i18n("HELP_SAD_DESCRIPTION", language),
    usage: i18n("HELP_SAD_USAGE", language)
  }
}