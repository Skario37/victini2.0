...
exports.run = async (client, message, args, settings) => {
  ...
  // Gfycat
  const dataGfycat = await gfycat.search({"search_text": "slap", "count": 10}).then(d => {
  ...
  // Giphy
  const dataGiphy = await giphy.search({"q": "slap", "limit": 20, "rating": "pg", "fmt": "json"}).then(d => {
  ...
  // Tenor
  const dataTenor = await Tenor.Search.Query("slap", "15").then(d => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_SLAP_SOMETHING", settings.language)
          .replace("{{emoji}}", Emoji.SLAP.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
          .replace("{{variable}}", args)
      );
    } else {
      embed.setTitle(
        i18n("GIF_SLAP_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.SLAP.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
...
exports.conf = {
  ...
  enabled: true,
  guildOnly: false,
  aliases: ["slam", "gifler", "claquer"]
}

exports.help = (language) => { 
  return {
    name: i18n("HELP_SLAP_NAME", language),
    category: i18n("HELP_SLAP_CATEGORY", language),
    description: i18n("HELP_SLAP_DESCRIPTION", language),
    usage: i18n("HELP_SLAP_USAGE", language)
  }
}