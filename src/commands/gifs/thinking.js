...
exports.run = async (client, message, args, settings) => {
  ...
  if (gifs.length) {
    ...
    if (args.length) {
      ...
      embed.setTitle(
        i18n("GIF_THINKING_YOURSELF", settings.language)
          .replace("{{emoji}}", Emoji.THINKING.text)
          .replace("{{author}}", message.member.nickname || message.member.user.username)
      );
    }
...
exports.help = (language) => { 
  return {
    name: i18n("HELP_THINKING_NAME", language),
    category: i18n("HELP_THINKING_CATEGORY", language),
    description: i18n("HELP_THINKING_DESCRIPTION", language),
    usage: i18n("HELP_THINKING_USAGE", language)
  }
}